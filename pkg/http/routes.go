...
func RegisterRoutes(mux *http.ServeMux, handler _h.Handler) {
	...
	mux.HandleFunc("/json/export", handleMethod("GET", handler.ExportTableToJson()))
	mux.HandleFunc("/csv/export", handleMethod("GET", handler.ExportTableToCSV()))
	mux.HandleFunc("/sql/export", handleMethod("GET", handler.ShowCreateTable()))
